...
public class SpecialEffectFire extends GameObject {
    ...
    public SpecialEffectFire(String[] path, int x, int y, int[] info) {
        super(x, y, info[0], info[1], info[2]);
        try {
            img1 = ImageIO.read(getClass().getResource(path[2]));
            img2 = ImageIO.read(getClass().getResource(path[3]));
        } catch (IOException e) {
            e.printStackTrace();
        }
